...
import httpx
import structlog
from bkjr.flows import FlowState, list_jobs, load_job, render_job_template, render_template
...
class SMSRecruitmentAgent:
    ...
    def __init__(self):
        self.quo = QuoClient()
        # Restore persisted candidate states from SQLite so a restart doesn't lose
        # the reply-tracking state built up from inbound webhooks.
        self.candidate_states: dict[str, dict] = load_candidate_states()
        log.info("candidate_states_loaded", count=len(self.candidate_states))
    ...
    def handle_inbound(self, webhook_payload: dict) -> dict:
        ...
        log.info("inbound_sms", from_number=from_number, message_id=message_id,
                 message=msg_body[:50])

        # ── Opt-out / opt-in: BEFORE any flow logic ──────────────────────────
        # This has to run ahead of FlowState.next_action, otherwise a "STOP"
        # would be classified as an ordinary reply and could trigger an
        # auto-response to someone who just told us to stop messaging them.
        intent = classify_inbound(msg_body)
        if intent == "stop":
            return self._handle_opt_out(from_number, to_number_id)
        if intent == "start":
            return self._handle_opt_in(from_number)
        ...
        # Checkpoint to SQLite BEFORE returning — so a restart can't lose this update
        checkpoint_candidate_state(from_number, json.dumps(candidate))
    ...
    def _handle_opt_out(self, from_number: str, to_number_id: str) -> dict:
        """
        Record the opt-out and send EXACTLY ONE confirmation, ever.

        record_opt_out() returns True only on the first insert, so a candidate
        who sends STOP three times gets one confirmation — sending three would
        itself be a compliance problem.
        """
        newly_recorded = record_opt_out(from_number, reason="inbound STOP keyword")
        confirmation_sent = False

        if newly_recorded:
            try:
                # allow_opted_out=True is required and correct here: the
                # recipient has, by definition, just opted out. This is the
                # only caller in the codebase permitted to set it.
                self.quo.send_sms(
                    to_number_id or QUO_BK_NUMBER_ID,
                    from_number,
                    OPT_OUT_CONFIRMATION,
                    allow_opted_out=True,
                )
                confirmation_sent = True
                log.info("opt_out_confirmation_sent", phone=from_number)
            except Exception as e:
                # The opt-out itself is already durably recorded — that is the
                # part that matters legally. A failed confirmation must not
                # roll it back.
                log.error("opt_out_confirmation_failed", phone=from_number, error=str(e))
        else:
            log.info("opt_out_repeat_no_confirmation", phone=from_number,
                     reason="already opted out — not re-confirming")

        candidate = self.candidate_states.get(from_number, {"phone": from_number})
        candidate["state"] = "opted_out"
        candidate["opted_out_at"] = utc_now_iso()
        self.candidate_states[from_number] = candidate
        checkpoint_candidate_state(from_number, json.dumps(candidate))
        self._sync_to_sheets(candidate)

        return {"handled": True, "action": "opt_out", "phone": from_number,
                "newly_recorded": newly_recorded, "confirmation_sent": confirmation_sent}

    def _handle_opt_in(self, from_number: str) -> dict:
        """START / UNSTOP — clear the opt-out so outreach may resume."""
        was_opted_out = clear_opt_out(from_number)
        candidate = self.candidate_states.get(from_number)
        if candidate and candidate.get("state") == "opted_out":
            candidate["state"] = "new"
            self.candidate_states[from_number] = candidate
            checkpoint_candidate_state(from_number, json.dumps(candidate))
            self._sync_to_sheets(candidate)
        log.info("opt_in_processed", phone=from_number, was_opted_out=was_opted_out)
        return {"handled": True, "action": "opt_in", "phone": from_number,
                "was_opted_out": was_opted_out}
    ...
    def trigger_screening_call(self, candidate: dict, job) -> dict:
        ...
        # 0. Run the dial guards BEFORE the pre-notice SMS. Step 1 sends a text
        #    and step 2 sleeps 10s, so discovering an opt-out or a closed
        #    calling window inside create_phone_call would be too late: the
        #    candidate would already have been told a call was coming.
        #    create_phone_call re-runs these — that is the real enforcement
        #    point; this call is ordering hygiene.
        try:
            self.retell.dial_preflight(candidate["phone"], candidate=candidate)
        except (OptedOutError, CallWindowError) as e:
            log.warning("screening_call_refused", phone=candidate.get("phone"),
                        error=str(e), error_type=type(e).__name__)
            return {"error": str(e),
                    "refused_by": type(e).__name__,
                    "next_ok_local_time": getattr(e, "next_ok_local_time", None)}
        ...
        result = self.retell.create_phone_call(
            ...
            candidate=candidate,   # timezone resolution for the TCPA window check
        )
